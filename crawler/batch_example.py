...
5. 生成的2条数据主题都为打铁花，但是内容及评论不能完全相同，且应主要是正面情感
6. 生成的5条数据中，评论数需要和实际生成的评论文本数量一致，如果没有评论，则评论数为0，如果评论数大于60，则取前60条评论
7. 发布日期在2026-01-01至2026-01-10之间
8. 请以JSON格式返回，格式为：{{"datas": [生成的5条数据]}}
"""
    return prompt
